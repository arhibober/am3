...
class CfactionDbRecordLoader{
	...
	function run($form, $actiondata){
		...
		if(!empty($table_name) && (trim($params->get('dbfield', '')) || trim($actiondata->content1))){
			...
			$where = trim($actiondata->content1) ? $this->_processWhere(trim($actiondata->content1)) : "`".$params->get('dbfield', '')."` = '".$form->data[$params->get('request_param', '')]."'";
			//load the model_id
