...
class SIGPlusImageServicesConfiguration {
	...
	public function checkFolders() {
		...
		if (preg_match('#^(?:[a-zA-Z]+:)?[/\\\\]#', $this->imagesfolder)) {  // starts with a leading slash (UNIX) or a drive letter designation and a backslash (Windows)
			...
			if ($this->baseurl === false && strpos($path, JPATH_ROOT.DS) === 0) {  // starts with Joomla root folder
				$this->baseurl = JURI::base(true).str_replace(DS, '/', substr($path, strlen(JPATH_ROOT)));
			}
		} else {
...
class SIGPlusImageServices {
	...
	*/
	public function getImagePath($entry) {
		if ($entry) {
			return $this->config->imagesfolder.DS.str_replace('/', DS, $entry);  // replace '/' with platform-specific directory separator
		} else {
	...
	*/
	private function getFullsizeImagePath($imagepath) {
		...
		$fullsizepath = dirname($imagepath).DS.str_replace('/', DS, $this->config->fullsizefolder).DS.basename($imagepath);
	...
	private function getImageShortUrl($imagepath) {
		if (strpos($imagepath, $this->config->imagesfolder.DS) === 0) {
			// cut off image base folder and normalize directory separator
			$segments = explode(DS, substr($imagepath, strlen($this->config->imagesfolder) + 1));
	...
	*/
	public function checkWatermarkPath($imagedirectory) {
		...
		$watermarkingallery = $imagedirectory.DS.$watermarkimage;
		// look inside watermark subfolder of image gallery folder (e.g. "images/stories/myfolder/watermark")
		$watermarkinsubfolder = $imagedirectory.DS.str_replace('/', DS, $this->config->watermarkfolder).DS.$watermarkimage;
		// look inside base path (e.g. "images/stories")
		$watermarkinbase = $this->config->imagesfolder.DS.$watermarkimage;
	...
	*/
	private function checkGeneratedImagePath($generatedfolder, $imageref, SIGPlusPreviewParameters $params) {
		if ($this->config->thumbscache || is_remote_path($imageref)) {  // images are set to be generated in cache folder OR image is at a remote location
			...
			$path = $directory.DS.$this->getImageHash($imageref, $params);  // hash original image file paths to avoid name conflicts
			if (is_file($path)) {
				return $path;
			}
		} else {  // an absolute file system path
			$directory = dirname($imageref).DS.str_replace('/', DS, $generatedfolder);
			$path = $directory.DS.basename($imageref);
			if (is_file($path)) {
				return $path;
			}
			if ($subfolder = self::getGeneratedSubfolder($params)) {
				$path = $directory.DS.$subfolder.DS.basename($imageref);
				if (is_file($path)) {
					return $path;
				}
			}
	...
	*/
	private function createGeneratedImagePath($generatedfolder, $imageref, SIGPlusPreviewParameters $params) {
		if ($this->config->thumbscache || is_remote_path($imageref)) {  // images are set to be generated in cache folder OR image is at a remote location
			...
			return $directory.DS.$this->getImageHash($imageref, $params);  // hash original image file paths to avoid name conflicts
		} else {  // an absolute file system path
			$directory = dirname($imageref).DS.str_replace('/', DS, $generatedfolder);
			...
			$this->createDirectoryOnDemand($directory);
			return $directory.DS.basename($imageref);
		}
	}
	...
	*/
	private function getGeneratedImageUrl($generatedfolder, $imageref, SIGPlusPreviewParameters $params) {
		if ($this->config->thumbscache || is_remote_path($imageref)) {  // images are set to be generated in cache folder OR image is at a remote location
			...
			if (strpos($imageabspath, $this->config->imagesfolder.DS) === 0) {  // does not walk outside image base folder
				$imagerelpath = substr($imageabspath, strlen($this->config->imagesfolder) + 1);  // cut off image base folder
				return $this->config->baseurl.'/'.pathurlencode(str_replace(DS, '/', $imagerelpath));
			}
		}
		return false;
	}
	...
	public function getListing($imagedirectory, $sortcriterion = SIGPLUS_FILENAME, $sortorder = SIGPLUS_ASCENDING, $depth = 0) {
		...
		foreach ($specialentries as $value) {
			if ($value !== false) {
				...
				$exceptions[] = str_replace('/', DS, $value);
			}
		}
	...
	*/
	private function getLabelsFilePath($imagedirectory, $labelsfilename) {
		...
		if ($this->config->multilingual) {  // check for language-specific labels file
			...
			$file = $imagedirectory.DS.$labelsfilename.'.'.$lang->getTag().'.txt';
			if (is_file($file)) {
				return $file;
			}
		}
		...
		$file = $imagedirectory.DS.$labelsfilename.'.txt';  // filesystem path to labels file
		if (is_file($file)) {
			return $file;
		}
		return false;
		...
		foreach ($matches as $match) {
			...
			if ($imagefile == '*') {  // set default label
				...
				if (is_remote_path($imagefile)) {  // a URL to a remote image
					...
					$imagefile = file_exists_case_insensitive($imagedirectory.DS.$imagefile);
	...
	*/
	public function getCachedContentPath($key, $suffix) {
		if ($this->config->contentcache) {  // use cache folder
			$directory = JPATH_CACHE.DS.str_replace('/', DS, $this->config->scriptfolder);
			return $directory.DS.$key.$suffix;
		}
		return false;
	}
	...
	*/
	public function saveCachedContent($imagebase, SIGPlusGalleryParameters $params, $code) {
		if ($this->config->contentcache) {
			$cachedirectory = JPATH_CACHE.DS.str_replace('/', DS, $this->config->scriptfolder);
	...
	*/
	public function cleanCachedContent() {
		if ($this->config->contentcache) {
			$dir = JPATH_CACHE.DS.str_replace('/', DS, $this->config->scriptfolder);
			if ($dh = @opendir($dir)) {
