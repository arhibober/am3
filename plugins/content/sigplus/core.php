...
class SIGPlusCore {
    ...
    public function __construct(SIGPlusConfiguration $configuration = null) {
		...
		if (SIGPLUS_DEBUG) {  // force debug mode
			...
			$engineservices->debug = (bool) $configuration->core->debug;  // do not disable debug mode if already set
		}
	...
	*/
	private function getPicasaImageGallery($url, $galleryid) {
		...
		// get thumbnail that best matches expected preview image dimensions
		$maxsize = min($this->curparams->width, $this->curparams->height);
		$size = reset($sizes);
		foreach ($sizes as $cursize) {
			// choose maximum size thumbnail that fits preview image dimensions
			if ($maxsize >= $cursize && $cursize >= $size) {
				$size = $cursize;
			}
		}
		...
		$items = array();
		$index = 0;
		foreach ($sxml->entry as $entry) {  // enumerate album entries
			...
			$thumbwidth = (int) $attrs['width'];
			$thumbheight = (int) $attrs['height'];
	...
	private function getUserDefinedImageGallery($imagedirectory, array $list, $galleryid) {
		foreach ($list as $listitem) {
			if (is_string($listitem)) {
				$this->createPreviewImage($imagedirectory.DS.$listitem);
			} else {
				$this->createPreviewImage($imagedirectory.DS.$listitem->imagefile);
			}
		}
		...
		foreach ($list as $listitem) {
			if (is_string($listitem)) {
				$image = $this->getImageData($imagedirectory.DS.$listitem, count($images));
			} else {
				$image = $this->getImageData($imagedirectory.DS.$listitem->imagefile, count($images), $listitem);
			}
			if ($image === false) {
				continue;
			}
	...
	*/
	private function getLabeledImageGallery($imagedirectory, $files, $labels, $galleryid) {
		...
		foreach ($files as $file) {
			$this->createPreviewImage($imagedirectory.DS.$file);
		}

		$images = array();
		foreach ($files as $file) {
			if (($image = $this->getImageData($imagedirectory.DS.$file, count($images), array_value($labelmap, $file))) === false) {
				continue;
			}
			$images[] = $image;
		}
	...
	*/
	private function getImageGalleryHtml($body, $paramstring = '') {
		...
		if ($this->curparams->link !== false) {
			$lightbox = $engineservices->getLightboxEngine($this->curparams->lightbox);  // get selected lightbox engine if any or use default
			if ($lightbox && ($linkscript = $lightbox->getLinkScript($this->curparams->link)) !== false) {
	...
	* Relative paths are interpreted w.r.t. the image base folder.
	* @param string $paramstring A whitespace-separated list of name="value" parameter values.
	*/
	public function getGalleryHtml($body, $paramstring = '') {
		if (!isset($this->imageservices)) {  // global error, image services are not available
			throw new SIGPlusInitializationException();
		}
		$oblevel = ob_get_level();
		try {
			return $this->getImageGalleryHtml($body, $paramstring);
