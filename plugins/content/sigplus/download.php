...
$imagepathinfo = http_path_info_url();
if (empty($imagepathinfo))
	http_critical_error('The image to download has not been specified in the URL.');
	
// check image existence
$imagepath = JPATH_ROOT.DS.str_replace('/', DS, $imagepathinfo);
$filename = basename($imagepath);
if (!is_file($imagepath))  // image file not found
	http_not_found($filename);
if (substr($imagepath, 0, strlen(JPATH_ROOT.DS)) !== JPATH_ROOT.DS)  // image path is outside Joomla folder
	http_not_found($filename);
